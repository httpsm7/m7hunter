#!/usr/bin/env python3
# core/pipeline_v5.py — M7Hunter v5.0 Pipeline
# Dual-Phase | Confidence Scoring | Proof Engine | Async-ready
...
PHASE1_STEPS = [
    ...
    "takeover","github","idor","redirect"
]

PHASE2_STEPS = [
    "ports","crawl","sqli","cors","lfi","screenshot",
    "wpscan","cloud","ssti","jwt","graphql","host_header","xxe","smuggling"
]
...
class PipelineV5:
    """
    M7Hunter v5.0 — Dual-Phase Pipeline

    Phase 1 (Fast): Lightweight signal detection
    Phase 2 (Deep): Confirmation of suspicious endpoints only

    Every finding goes through:
    1. OfflineAI FP filter
    2. ConfidenceEngine scoring
    3. ProofEngine (if confirmed)
    4. RiskScorer (CVSS-like)
    """
    ...
    def __init__(self, target, args, tor, oob, notifier, log,
        ...
        self.findings            = []
        self.suspicious_endpoints= []   # Phase-1 suspects → Phase-2 targets
        ...
        self.out = os.path.join(base, f"{self.prefix}_{ts}_v5")
        ensure_dir(self.out)
    ...
    def run(self) -> str:
        ...
        self.log.info(f"Offline AI: {'ON' if self.offline_ai else 'OFF'}")
        print()
        ...
        # ── Phase 2: Deep confirmation (only if --deep) ───────────────
    ...
    def add_finding(self, severity, vuln_type, url, detail="", tool="",
        ...
        """
        V5 enhanced add_finding:
        1. Dedup check
        2. Offline AI FP filter
        3. Confidence scoring
        4. Status assignment (confirmed/potential/false_positive)
        """
        # Dedup
        ...
        # Skip if below threshold and not confirmed
        if conf_score < self.confidence_threshold and status != "confirmed":
            if conf_score < 0.4:
                return  # Discard very low confidence
        ...
        entry = {
            ...
            "ai_analysis"     : ai_analysis,
            "risk_score"      : None,  # filled by _calculate_risk_scores
            "proof"           : None,  # filled by _generate_proofs
        }

        with self._findings_lock:
            self.findings.append(entry)

        # Track suspicious endpoints for Phase-2
        if status == "potential" and url not in self.suspicious_endpoints:
            self.suspicious_endpoints.append(url)

        # Observer + audit
        self.observer.record_finding(severity, vuln_type, url, detail, tool)
        self.audit.log_finding(entry)

        # Notify critical/high immediately
        if self.notifier:
            self.notifier.send_finding(severity, vuln_type, url, detail, tool)

    def add_suspicious(self, url, reason=""):
        """Mark endpoint as suspicious for Phase-2 deep scan."""
        if url not in self.suspicious_endpoints:
            self.suspicious_endpoints.append(url)
            self.log.info(f"  [P1→P2] Suspicious: {url[:60]} | {reason}")

    # ── Proof generation ─────────────────────────────────────────────
    def _generate_proofs(self):
        """Generate curl command + raw HTTP for every confirmed finding."""
    ...
    def _calculate_risk_scores(self):
        """Assign CVSS-like risk score to every finding."""
        ...
        sorted_findings = sorted(
            self.findings,
            key=lambda f: f.get('risk_score', {}).get('total', 0),
            reverse=True
        )
