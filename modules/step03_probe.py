...
class Step03Probe:
    ...
    def run(self):
        ...
        # Format output
        # FIX: safe access — keys may not exist in all pipeline configs
        if p.files.get("fmt_url"):  FormatFixer.fix(out, p.files["fmt_url"],  "url")
        if p.files.get("fmt_host"): FormatFixer.fix(out, p.files["fmt_host"], "host")
